...
function openRecipeModal(id=null) {
  ...
  if(r.nutrition) {
    ...
    setV('r-bake-min', r.bakeMin); setV('r-bake-temp', r.bakeTempC); setV('r-pieces-per-tray', r.piecesPerTray);
    ...
    ['r-name','r-base-portion','r-bake-loss','r-unit-weight','r-temp1','r-time1',
     ...
     'r-setup-min','r-per-unit-min','r-bake-min','r-bake-temp','r-units-per-tray','r-trays-per-cycle','r-mixer-min','r-pieces-per-tray'].forEach(x=>{
